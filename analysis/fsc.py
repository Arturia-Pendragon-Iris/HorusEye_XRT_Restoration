...
def get_slices(d):
    ...
    if d == 2:
        ...
        slices = [
            [[slice(None), slice(None), slice(None, None, 2)], [slice(None), slice(None), slice(1, None, 2)]],
            # split column
            [[slice(None), slice(None, None, 2), slice(None)], [slice(None), slice(1, None, 2), slice(None)]],
            # split row
            [[slice(None, None, 2), slice(None), slice(None)], [slice(1, None, 2), slice(None), slice(None)]]
            # split layer
        ]
...
def compute_self_fsc(img, voxel=1):
    r = img.shape[0] // 2
    sfsc = np.mean(single_image_frc(img, r), axis=0)
    sfsc_adjusted = 2 * sfsc / (1 + sfsc)

    freq = get_radial_spatial_frequencies(img, voxel, mode='full')
    return sfsc_adjusted


def split_3D_matrix(matrix):
    n = matrix.shape[-1]
    numbers = list(range(0, n))
    random.shuffle(numbers)

    group1 = [2 * x for x in range(0, n // 2)]
    group2 = [2 * x + 1 for x in range(0, n // 2)]

    if len(matrix.shape) == 2:
        matrix1 = matrix[:, group1]
        matrix2 = matrix[:, group2]
    else:
        matrix1 = matrix[:, :, group1]
        matrix2 = matrix[:, :, group2]
...
def compute_fsc(img, voxel=1):
    y1, y2 = split_3D_matrix(img)
    r = y1.shape[0] // 2
    fsc = two_volume_fsc(y1, y2, r)
    print(y1.shape, y2.shape)
    freq = get_radial_spatial_frequencies(img, voxel, mode='full')
    return fsc, freq


def compute_two_fsc(img_1, img_2, voxel=1):
    r = img_1.shape[0] // 2

    fsc = two_volume_fsc(img_1, img_2, r)
    # print(y1.shape, y2.shape)
    freq = get_radial_spatial_frequencies(img_1, voxel, mode='full')
    return fsc, freq
