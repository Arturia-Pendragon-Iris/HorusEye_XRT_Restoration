...
class DIPReconstructor(IterativeReconstructor):
    HYPER_PARAMS = {
        'lr': {'default': 1e-3, 'range': [1e-5, 1e-1]},
        'gamma': {'default': 1e-4, 'range': [0, 1e-0], 'grid_search_options': {'num_samples': 20}},
        'scales': {'default': 4, 'choices': [3, 4, 5, 6, 7]},
        'channels': {'default': [128] * 5},
        'skip_channels': {'default': [4] * 5},
        'iterations': {'default': 15000, 'range': [1, 50000]},
    ...
    def __init__(self, ray_trafo, hyper_params=None, callback=None,
        ...
        self.ray_trafo_module = OperatorModule(ray_trafo)
    ...
    def _reconstruct(self, observation, *args, **kwargs):
        ...
        self.net_input = get_noise(input_depth, 'noise', input_shape).to(device)

        self.model = skip(
            input_depth, output_depth,
            num_channels_down=channels[:scales],
            num_channels_up=channels[:scales],
            num_channels_skip=skip_channels[:scales],
            upsample_mode='bilinear'
        ).to(device)

        self.optimizer = Adam(self.model.parameters(), lr=lr)

        y_target = torch.tensor(observation.asarray(), dtype=torch.float32).view(1, 1, *observation.shape)
        ...
        for i in range(iterations):
            ...
            recon_loss = criterion(self.ray_trafo_module(output), y_target)
            reg_loss = gamma * tv_loss(output)
            loss = recon_loss + reg_loss
            # print(recon_loss.item(), reg_loss.item())
            loss.backward()
            self.optimizer.step()

            for param in self.model.parameters():
                param.data.clamp_(MIN, MAX)
        ...
        return best_output.cpu().numpy()[0, 0]
...
class DeepImagePriorReconstructor(IterativeReconstructor):
    ...
    def _reconstruct(self, observation, *args, **kwargs):
        ...
        for i in tqdm(range(iterations), total=iterations):
            ...
            print(criterion(self.ray_trafo_module(output),
                             y_delta), gamma * tv_loss(output))
            loss.backward()
